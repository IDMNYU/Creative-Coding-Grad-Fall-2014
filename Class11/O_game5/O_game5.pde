...
AudioPlayer[] groove = new AudioPlayer[8]; // romney saying half
AudioPlayer[] rgroove = new AudioPlayer[8]; // romney saying half

AudioPlayer introsound, scenesound;
...
int LEVEL = 3; // what level are we on?
...
void setup()
{
  ...
  // CONTROLLER
  mitt = new lukeBox(width/2, height/2, 30, 30, "mitt.jpg");

  // Make the spring (it doesn't really get initialized until the mouse is clicked)
  spring = new lukeSpring();
  spring.bind(width/2, height/2, mitt);
  ...
  {
    rgroove[i] = minim.loadFile("likeaplant.mp3", 2048);
  }
  
  introsound = minim.loadFile("ninebucks.mp3", 2048);
  scenesound = minim.loadFile("theworst.mp3", 2048);
}
