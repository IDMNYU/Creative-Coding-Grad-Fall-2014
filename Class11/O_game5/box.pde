...
class lukeBox
{
  ...
  lukeBox(float x, float y, float _w, float _h, String imagename)
  {
    spritepic = loadImage(imagename);
    w = _w;
    h = _h;
    r = random(128, 255);
    g = random(128, 255);
  ...
  void bink()
  {
    body.getFixtureList().setFriction(10000.);
  }
...
class lukeBoss
{
  ...
  // this is the constructor for the class
  lukeBoss(float x, float y, float _w, float _h, String imagename)
  {
    spritepic = loadImage(imagename);
    w = _w;
    h = _h;
